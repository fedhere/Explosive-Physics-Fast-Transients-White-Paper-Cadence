...
\begin{center}
\begin{figure}[!h]
\includegraphics[width=8cm,height=8cm]{figures/.pdf}
\includegraphics[width=8cm,height=8cm]{figures/.pdf}
\caption{{\footnotesize PLACEHOLDER FOR MRD'S PLOTS}. }}
\end{figure}
\end{center}
...
The prompt characterization of fast transients and fast features of transients, which enables the triggering of crucial follow up observations, requires the determination of \emph{both color and lightcurve shape}. This, in turn, requires observations in 2 different filters within a short interval of time \dtone\ and to return to the same field with one of those filters at a later time \dttwo. The constraints on these timeline are:
\begin{enumerate}
    \item max(\dtone )
    \item min(\dttwo )
